...
from utils import get_sents_lenth3
...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        scores = target_gold_words_log_prob.sum(dim=0) + target_ox_log_prob.sum(dim=0)
        return scores
    ...
    def parallel_decode(self,target, tgt,tgt_padded):
        ...
        sbol = [['(', ')', ',', "'", '"','_'],'_']
        if type(target[0]) is not list: target = [target]
        Z, XO = get_sents_lenth3(target,sbol) # Z:각 sentence 내의 각 어절의 길이로 구성  list[list]
    ...
    def parallel_beam_encode2(self, X, init_vecs= None):
        ...
        if init_vecs is not None:
            print(X_embed.shape, init_vecs.shape)
        else:
            print(X_embed.shape)
 
        out,(last_h1,last_c1) = self.sub_decoder(X_embed,init_vecs)
        X_proj = torch.tanh(self.sub_de_projection(out[0]))
        X_gate = torch.sigmoid(self.gate(X_embed[0]))
        X_way = X_gate * X_embed + (1-X_gate) * X_proj       
        
        return X_way, last_h1
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            if t==1:
                ...
                prev_init_vecs = torch.cat(prev_init_vecs,0).unsqueeze(0) 
            ...
            #xos = F.log_softmax(self.target_ox_projection(att_t), dim=-1).max(-1)[1]  # 추가된 부분
            log_px, xos = F.log_softmax(self.target_ox_projection(att_t), dim=-1)
            #next_init_vecs = next_init_vecs * torch.tensor(xos,dtype=torch.float,device=self.device)
            log_px = log_px.unsqueeze(-1).expand_as(log_p_t)

            live_hyp_num = beam_size - len(completed_hypotheses)
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(log_p_t) + log_px+log_p_t).view(-1)
            top_cand_hyp_scores, top_cand_hyp_pos = torch.topk(contiuating_hyp_scores, k=live_hyp_num)

            prev_hyp_ids = top_cand_hyp_pos / len(self.vocab.tgt)
            hyp_word_ids = top_cand_hyp_pos % len(self.vocab.tgt)

            next_init_vecs = torch.split(next_init_vecs.squeeze(0)[prev_hyp_ids],1,0)
            ...
            for prev_hyp_id, hyp_word_id, cand_new_hyp_score,new_init in zip(
                prev_hyp_ids, hyp_word_ids, top_cand_hyp_scores, next_init_vecs):
                ...
                if hyp_word == '</s>':
                    ...
                    prev_init_vecs.append(new_init)
