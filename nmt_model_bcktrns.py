...
class NMT(nn.Module):
    ...
    def en_encode(self, source_padded: torch.Tensor, source_lengths: List[int], self_trns=0): # -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #return self.dropout_30(enc_hiddens), dec_init_state
        
        if self_trns == 1:
            return self.dropout(enc_hiddens), dec_init_state
        else:
            return enc_hiddens, dec_init_state  
    ...
    def ek_beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70, tlang:str='en') -> List[Khypothesis]:
        ...
        """
        best_score = -1000000.
        for i,v in enumerate(completed_hypotheses): # 수정필요 !!!!
        ...
        """
        for ib in range(len(completed_hypotheses)):
            temp_h = []
            #pre_v = ''
            for i,v in enumerate(completed_hypotheses[ib].value):
               if i>0 and completed_hypotheses[ib].xo[i] == 0:
                  temp_h += [v]
               #elif i>0 and pre_v not in self.token and v not in self.token and completed_hypotheses[0].xo[i] == 0:
               #   temp_h += ['_'] + [v]
               else:
                  temp_h += [self.sbol[completed_hypotheses[ib].xo[i]-1]]+[v]
               #pre_v = v
            completed_hypotheses[ib] = Khypothesis(value=temp_h,
                                                 xo =  completed_hypotheses[ib].xo,
                                                 score= completed_hypotheses[ib].score)
        
        tr_sents = [completed_hypotheses[i].value for i in range(len(completed_hypotheses))]
        re_snts,re_scrs = self.greedy_search(tr_sents, 
                                max_decoding_time_step= 100, slang = tlang, tlang = slang, mapping=0)
        
        bleu_s1 = [corpus_bleu([b_s[0]],[b_s[1]]) for b_s in list(zip([s1 for s1 in tr_sents],[s2 for s2 in re_snts]))]
        bleu_s2 = [corpus_bleu([b_s[0]],[b_s[1]]) for b_s in list(zip([s2 for s2 in re_snts],[s1 for s1 in tr_sents]))]

        #bleu_s1 = corpus_bleu([s1 for s1 in tr_sents],[s2 for s2 in re_snts])
        #bleu_s2 = corpus_bleu([s2 for s2 in re_snts],[s1 for s1 in tr_sents])
        
        wt = 1
        comb_scores = [(i,completed_hypotheses[i].score/len(completed_hypotheses[i].xo)
                       + wt * bleu_s1[i] * bleu_s2[i] / (bleu_s1[i] + bleu_s2[i])) 
                       for i in range(len(completed_hypotheses))]
        i_rank = [i for (i,v) in sorted(comb_scores, key = lambda x:x[1], reverse=True)]
        
        re_ranked = [completed_hypotheses[i] for i in i_rank]
        
        
        return re_ranked   #completed_hypotheses
