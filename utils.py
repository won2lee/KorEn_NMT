...
def read_corpus(file_path, source):
    ...
    for line in open(file_path):
        ...
        line = p0.sub(' ',p2.sub('',p1.sub('\g<t1>',p1.sub('\g<t1>',p1.sub('\g<t1>','_ '+line)))))
...
def batch_iter(data, batch_size, slang, tlang, shuffle=False, mapping=0, self_trns=0):
    ...
    for i in range(batch_num):
        ...
        if slang == tlang:
            ...
            examples = src_sents
        """
        if slang == tlang or self_trns==1:
            src_sents = []
            for e in examples:
                s = e[0]
                s = p.sub(' ',p2.sub('',p1.sub('\g<t1>',p1.sub('\g<t1>',' '.join(s))))).strip().split(' ')
                ...
                while sl>0:
                    ...
                    #    s = s[1:]              
                        ...
                        noise_l = 12 if mapping==1 or self_trns ==1 else 6    # self trans 인 경우 12=> 6으로 노이즈 높임 mapping(번역하는 경우) 노이즈가 작게 (숫자 크게)
                ...
                sent = []
                k_unk = 20 if mapping==1 else 6  #5
                for i in range(sl):
                    w = snt[i]                    
                    if w in sbol:
                        k = np.random.randint(30)
                        wk = w if self_trns == 1 or k > 10 else sbol[k//10]
                        sent.append(wk)
