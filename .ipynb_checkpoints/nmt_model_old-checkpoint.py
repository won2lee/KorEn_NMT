...
from utils import get_sents_lenth
...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.sub_projection = nn.Linear(2*self.hidden_size, self.hidden_size, bias=False)         
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        #for i in range(2):
            #print(src_padded[:,i],XX[i],Z_len[i])
        ...
        #print("X after split : {}".format([len(s) for s in X]))
        Z_flat = list(chain(*Z_sub))
        X = [s[:Z_flat[i]]for i,s in enumerate(X)]
        #X = [s[:-1] if len(s)>1 else s for s in X]
        X = pad_sequence(X).squeeze(-1)
        X = torch.tensor(X,device = self.device)
        #print("X.shape after pad_sequence : {}".format(X.shape))
 
        X_embed = self.model_embeddings.source(X)
        #print("X.shape after embedding : {}".format(X.shape))

        #X = pack_padded_sequence(X, XX_flattened,enforce_sorted=False)
        X_gate = torch.sigmoid(self.gate(X_embed))
        #X_gate = X_gate * 0.
        out,(last_h1,last_c1) = self.sub_encoder(X_embed)
        #last_h = torch.cat((last_h1[0,:],last_h1[1,:],last_h2[0,:],last_h2[1,:]), -1)
        #alpha_w = torch.sigmoid(self.gate(X))
        #print(last_h.shape, alpha_w.shape)
        #out = alpha_w * out1 + (1- alpha_w) * out2
        #last_h = torch.cat((last_h[0,:],last_h[1,:]), -1)
        #X = self.sub_projection(last_h)
        X_proj = torch.relu(self.sub_projection(out))
        X_way = self.dropout(X_gate * X_proj + (1-X_gate) * X_embed)
        #X = pad_packed_sequence(out, batch_first=True)[0] 
        #print("X.shape after sub_projection : {}".format(X.shape))
        #X = torch.split(X,Z_len)
        ...
        #  print("X[i].shape after split : {}".format(X[i].shape))
        source_padded = pad_sequence(X_input).squeeze(-2)
        #print("X.shape after pad {}".format(source_padded.shape))
        #source_lengths = [sum([wl-1 if wl>1 else wl for wl in s]) for s in Z]
        #source_lengths = [sum([wl for wl in s]) for s in Z]
        source_lengths = [sum([wl for wl in s]) for s in Z_sub]
        #print("source_lengths after pad {}".format(source_lengths))
        """
